function [fwd_mesh,pj_error] = reconstruct_stnd_region(fwd_mesh,...
                                                      frequency,...
                                                      data_fn,...
                                                      iteration,...
                                                      lambda,...
...
%						  region)
%
% Reconstruction program for standard meshes using region based priori
...
for it = 1 : iteration
  ...
  N = [fwd_mesh.kappa'*K./sum(K) fwd_mesh.mua'*K./sum(K)];
  nn = length(N);
  ...
  % since we are using J'*J, we will regularise for mua and kappa
  % components.
  reg_kappa = max(diag(Hess(1:end/2,1:end/2))).*lambda;
  reg_mua = max(diag(Hess(end/2+1:end,end/2+1:end))).*lambda;
  
  reg = ones(ncol,1);
  reg(1:end/2) = reg_kappa;
  reg(end/2+1:end) = reg_mua;
  ...
  foo = [K*foo(1:end/2); K*foo(end/2+1:end)];
