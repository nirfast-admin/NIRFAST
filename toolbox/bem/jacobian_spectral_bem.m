function [J,data,mesh] = jacobian_spectral_bem(mesh,frequency,wv_array)

% [J,data,mesh] = jacobian_spectral_bem(mesh,frequency,wv_array)
%
...
data.wv = wv_array;
J = [];
J_small = zeros(ndata*2,nregions*(m+2));
for i = 1:nwv
    data_tmp(i).paa(end+1:ndata,:) = NaN;
    data.paa(:,i*2-1:i*2) = data_tmp(i).paa;
    
    J_mua = J_tmp(i).complete(:,1:nregions);
    J_kappa = J_tmp(i).complete(:,nregions+1:end);
    ...
    for ii = 1 : ndata*2
      J_small(ii,m*nregions+1:(m+1)*nregions) = J_kappa(ii,:).*sa_factor';
      J_small(ii,(m+1)*nregions+1:(m+2)*nregions) = J_kappa(ii,:).*sp_factor';
    end
